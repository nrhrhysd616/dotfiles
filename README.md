...
| GitHub Actions（`.github/workflows/gitleaks.yml`） | プルリクエストの差分 | 飛ばせない |

CIをプルリクエストだけに絞っているのは、pushでも走らせると同じコミットを二度検査することになり、
かといってmainへのpushに絞ると検出がマージ後になって遅いためです。マージ前に止めるのが目的なので、
**mainへ直接pushした場合はCIが走りません**（その場合の防壁はローカルのフックだけになります）。
...
**既存の端末では`init-mac.zsh`を再実行してください。** gitleaks本体のインストールと
`~/.git-hooks`のリンクはそこで行われます。`git/.gitconfig`はシンボリックリンクなので
pullした時点で`core.hooksPath`は効きますが、リンク先が無い間はフックが存在しない扱いになり、
**警告も出ないまま検査だけが走らない状態**になります。次のコマンドで確認できます。

```zsh
ls -l ~/.git-hooks && gitleaks version
```
...
`copy`はファイルや標準入力の内容をクリップボードへ入れるコマンドです。
`pbcopy < README.md`のようにリダイレクトを書き分けなくても、`copy README.md`とパスを渡すだけで済みます。

クリップボードへの書き込みには環境に応じたコマンドが選ばれます（macOSは`pbcopy`、Waylandは`wl-copy`、
X11は`xclip`か`xsel`、WSLは`clip.exe`）。特定のOS専用ではないため`mac-`のようなプレフィックスは付いていません。

- **ファイルをコピーする**

  ```zsh
  copy README.md              # 1つのファイル
  copy src/a.ts src/b.ts      # 複数指定すると連結される
  copy -H src/*.ts            # 各ファイルの前に "==> src/a.ts <==" の見出しを付ける
  ```

  複数のコードをAIやチャットへ貼るときは`-H`を付けると、貼った先でもファイルの境目が残ります。

- **コマンドの出力をコピーする**

  ```zsh
  git diff | copy             # 標準入力から読む
  copy - README.md < head.txt # "-" は標準入力を表す。ファイルと混ぜられる
  ```

- **その他**

  ```zsh
  copy -q README.md           # 完了行を出さない
  copy --help                 # 使い方
  ```

  コピーすると`✓ copied 42 lines, 1.2 KB from README.md`のように結果が1行出ます。
  この表示もエラーもすべて標準エラーへ出るため、標準出力を使うパイプは汚れません。

補足:

- 指定したパスは**すべて検査してから**書き込みます。`copy a.txt typo.txt`のように一部が読めない場合は
  何もコピーせずに終了するので、その時点のクリップボードは壊れません
- **空の入力は拒否します。** 差分の無い`git diff | copy`でクリップボードが黙って空になり、
  直前にコピーした内容を失うのを避けるためです（`pbcopy`はこの場合クリップボードを空にします）
- 貼り付け側のコマンドは用意していません。`paste`はPOSIX標準コマンド（`/usr/bin/paste`、複数ファイルの行を
  横に連結する）として実在し、`$HOME/.local/bin`がPATHの先頭にあるため、同名を置くと標準の`paste`を隠します。
  貼り付けは`pbpaste`を使ってください
